...
func TestGenerateActionCommandCreatesScaffold(t *testing.T) {
	root := t.TempDir()
	writeCLIProjectRoot(t, root)
	writeCLIGoModule(t, root, "example.com/acme")
	writeCLIApp(t, filepath.Join(root, "apps", "sales"), "sales")
...
`)
	...
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := Run(context.Background(), []string{"generate", "action", "sales/lead"}, strings.NewReader(""), &stdout, &stderr)
	if err != nil {
		t.Fatalf("Run(generate action) error = %v, want nil", err)
	}
	for _, want := range []string{
		"generated action for sales/lead",
		"action: apps/sales/entities/lead/actions/actions.go (created)",
		"runner: cmd/dygo/main.go (created)",
	} {
		if !strings.Contains(stdout.String(), want) {
			t.Fatalf("generate action stdout = %q, want substring %q", stdout.String(), want)
		}
	}
	for _, path := range []string{
		"apps/sales/entities/lead/actions/actions.go",
		"cmd/dygo/main.go",
	} {
		if _, err := os.Stat(filepath.Join(root, filepath.FromSlash(path))); err != nil {
			t.Fatalf("Stat(%s) error = %v, want generated file", path, err)
	...
	actionSource := readCLIFile(t, filepath.Join(root, "apps", "sales", "entities", "lead", "actions", "actions.go"))
	if strings.Contains(actionSource, "DO NOT EDIT") || strings.Contains(actionSource, "Code generated by dygo") {
		t.Fatalf("generated action source = %q, want developer-owned starter without generated ownership header", actionSource)
	}

	stdout.Reset()
	stderr.Reset()
	if err := Run(context.Background(), []string{"hook", "validate"}, strings.NewReader(""), &stdout, &stderr); err != nil {
		t.Fatalf("Run(hook validate) error = %v, want generated action wiring valid", err)
	}
}

func TestGenerateActionDryRunDoesNotWrite(t *testing.T) {
	root := t.TempDir()
	writeCLIProjectRoot(t, root)
	writeCLIGoModule(t, root, "example.com/acme")
	writeCLIApp(t, filepath.Join(root, "apps", "sales"), "sales")
...
`)
	...
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	err := Run(context.Background(), []string{"generate", "action", "sales/lead", "--dry-run"}, strings.NewReader(""), &stdout, &stderr)
	if err != nil {
		t.Fatalf("Run(generate action --dry-run) error = %v, want nil", err)
	}
	for _, want := range []string{
		"generated action for sales/lead",
		"action: apps/sales/entities/lead/actions/actions.go (would create)",
		"runner: cmd/dygo/main.go (would create)",
	} {
		if !strings.Contains(stdout.String(), want) {
			t.Fatalf("generate action --dry-run stdout = %q, want substring %q", stdout.String(), want)
		}
	}
	if _, err := os.Stat(filepath.Join(root, "apps", "sales", "entities", "lead", "actions", "actions.go")); !os.IsNotExist(err) {
		t.Fatalf("dry-run action stat error = %v, want missing generated file", err)
	}
}
