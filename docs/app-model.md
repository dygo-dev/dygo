...
Hooks are app-owned Go code inside Entity bundles. A file such as `entities/lead/hooks.go` belongs to Entity `lead`. Entity actions live in `entities/lead/actions/actions.go` so they keep a separate Go package. dygo validates these conventions, but the code must still be compiled into a project runner through `pkg/dygo/runtime`; dygo does not dynamically load Go source files.
