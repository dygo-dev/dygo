...
func newRouteListCommand(stdout io.Writer) *cobra.Command {
	return &cobra.Command{
		Use:   "list",
		Short: "List routeable Entities, Pages, and reserved root slugs",
		Args:  cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			for _, route := range routeplan.EntriesWithPages(metadata.Entities, metadata.Pages) {
				routePath := "/" + route.Slug
				if route.Slug == "" {
					routePath = "/"
				}
				owner := route.EntityName
				if route.Kind == "page" {
					owner = route.PageName
				}
				if _, err := fmt.Fprintf(stdout, "%s %s %s/%s %s\n", routePath, route.Kind, route.AppName, owner, relToWorkingRoot(route.Path)); err != nil {
...
func newRouteValidateCommand(stdout io.Writer) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			result, err := routeplan.ValidateWithPages(metadata.Entities, metadata.Pages)
			if err != nil {
			...
			if _, err := fmt.Fprintf(stdout, "routes are valid: %d reserved routes, %d entity routes, %d page routes, %d conflicts\n", result.ReservedRoutes, result.EntityRoutes, result.PageRoutes, result.Conflicts); err != nil {
...
func newRouteResolveCommand(stdout io.Writer) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			return writeRouteResolution(stdout, method, normalized, routeplan.EntriesWithPages(metadata.Entities, metadata.Pages))
		},
	}
}
...
func writeRouteResolution(stdout io.Writer, method string, requestPath string, routes []routeplan.Entry) error {
	...
	if len(parts) == 1 {
		if route, ok := findRouteBySlug(routes, parts[0]); ok {
			...
			return writeOwnedRoute(stdout, handler, route, "")
		}
	}
	if requestPath == "/" {
		if route, ok := findRouteBySlug(routes, ""); ok {
			return writeOwnedRoute(stdout, "page route", route, "")
		}
	}
...
func writeAPIResolution(stdout io.Writer, method string, parts []string, routes []routeplan.Entry) error {
	...
	if len(parts) >= 5 && parts[0] == "api" && parts[1] == "v1" && parts[2] == "entities" && parts[4] == "meta" {
		...
		return writeOwnedRoute(stdout, "api metadata", route, string(permissions.ActionRead))
	}
	if len(parts) >= 3 && parts[0] == "api" && parts[1] == "v1" {
		switch parts[2] {
		case "auth":
			return writeStaticRoute(stdout, "api auth", "authentication endpoint")
		case "apps", "boot", "entities", "pages", "platform":
...
func writeOwnedRoute(stdout io.Writer, handler string, route routeplan.Entry, action string) error {
	...
	if _, err := fmt.Fprintf(stdout, "%s: %s/%s\n", ownerKind, route.AppName, owner); err != nil {
		return fmt.Errorf("write route resolve output: %w", err)
	}
